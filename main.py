...
class CustomPDF(FPDF):
    def __init__(self):
        # Page size: width=210mm (A4), height=360mm (taller)
        super().__init__(format=(210, 360))
        self.set_auto_page_break(False)

    def header_section(self, x, y, w, title):
        self.set_xy(x, y)
        self.set_font('DejaVu', 'B', 14)
        self.set_text_color(40, 40, 90)  # Dark Blue
        self.cell(w, 8, f"{title}:", ln=1)
        # Colored line under header
        line_y = self.get_y() - 2
        self.set_draw_color(40, 40, 90)
        self.set_line_width(1)
        self.line(x, line_y, x + w * 0.3, line_y)
...
def render_pdf_from_data(context):
    ...
    if image_url.startswith("data:image"):
        try:
            ...
            with NamedTemporaryFile(delete=False, suffix=".png") as tmpfile:
                img.save(tmpfile.name)
                pdf.image(tmpfile.name, x=pdf.w / 2 - 25, y=margin_top, w=50, h=50)
                os.unlink(tmpfile.name)
        except:
            pass

    pdf.set_y(margin_top + 60)
    ...
    # Vertical line separating columns (full page height minus margins)
    top_y = pdf.get_y()
    bottom_y = pdf.h - pdf.b_margin
    pdf.set_draw_color(200, 200, 200)
    ...
    # Sections for left and right columns
    left_sections = [
        ("About Me", context.get("about_me", ""), False),
        ("Education", context.get("education", "").split("\n"), True),
        ("Interests", context.get("interests", "").split("\n"), True),
    ...
    # Draw left sections fully on page 1
    def draw_section_full(x, y, title, content, is_list):
        pdf.header_section(x, y, col_width, title)
        y = pdf.get_y() + 2
        pdf.set_xy(x, y)
        if is_list and isinstance(content, list):
            for item in content:
    ...
    # Draw right sections with page split if needed
    def draw_section_split(x, y, title, content, is_list):
        line_height = 6

        def draw_title(y_pos):
            pdf.header_section(x, y_pos, col_width, title)
